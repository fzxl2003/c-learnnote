/*
字符串输出函数  puts(),fputs(),printf()，sprintf()
...
3.函数结尾输出换行符
 ...
 fgets()
1.函数原型： char *fgets(const char *s,FILE *fp);
2.函数参数既可以为一个字符串常量，也可以是字符数组(数组必须是字符串，以\0结尾)
3.函数结尾不自动输出换行符
...
2.从标准输入上读入一个连续的字符序列，直至遇到换行符或结尾
 并将其保存到参数所指定的字符数组s[]中
 结尾加'\0',不加换行符
 执行错误时，返回NULL

 fgets()
1.函数原型：char *fgets(char s[],int n,FILE *fp)；
2.char s[]与gets()的参数一致
 int n用于指定保存读入数据的字符数组的长度
 FILE *fp用于说明读入数据的来源
3.从标准输入上读入一个连续的字符序列，直至遇到换行符或结尾
 并将其保存到参数所指定的字符数组s[]中，结尾加'\0'
4.与gets()的不同点
...
2.功能：将数组src[]中的字符串复制到dest[]中已有的字符串后面，并返回dest[]
       dest[]中必须保存有以'\0'的字符串
...
2.功能：若字符串s中存在字符c，strchr()和strrstr()分别返回字符c在s中第一次和最后一次出现的位置。
      若字符串s中没有字符c，则返回NULL
