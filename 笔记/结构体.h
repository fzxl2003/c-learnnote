...
一.结构体的定义、赋值与使用
 ...
 定义方式：
  ...
  3.结构体变量的调用
    ...
    3.同一结构体的结构体变量可以相互整体赋值 如stu1=stu2;
    4.不可以整体输入输出，不能直接参与运算，不能整体赋值，只能单独去调用  如stu1.num
    5.在函数中的使用
       作为参数  函数声明  int a(struct stu stu1);         //往往向函数传递结构体变量的指针，效率更高
       作为返回值 函数声明 struct stu a();  //函数返回结构体stu的值
二.结构体数组（结构体变量为数组的情况）
 1.变量的定义：struct stu{
                       char *name;  //姓名
                          int num;  //学号
